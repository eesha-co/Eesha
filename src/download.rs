...
async fn download_to_path(
    ...
    let _ = eesha_internal_sender.send(EeshaInternalMsg::CreateDownload(download));

    let mut wrote_bytes: usize = 0;
    let mut last_update = tokio::time::Instant::now();

    send_update_to_eesha(
...
fn send_update_to_eesha(
    ...
    let _ = eesha_internal_sender.send(EeshaInternalMsg::UpdateDownload(
        download_id.clone(),
        UpdateDownloadState {
            status,
            progress,
