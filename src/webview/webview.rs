...
use url::Url;
use eesha_messages::ToControllerMessage;
...
use crate::{
    ...
    app::{EeshaInternalMsg, send_to_constellation},
    webview::{
...
impl Window {
    ...
    pub fn handle_servo_messages_with_webview(
        ...
        log::trace!("Eesha WebView {webview_id:?} is handling Embedder message: {message:?}",);
        match message {
            EmbedderMsg::WebViewClosed(_) => {
                // Most WebView messages are ignored because it's done by compositor.
                log::trace!("Eesha WebView {webview_id:?} ignores this message: {message:?}")
            }
            ...
            EmbedderMsg::WebViewFocused(w) => {
                ...
                log::debug!(
                    "Eesha Window {:?}'s webview {} has loaded completely.",
                    self.id(),
                    w
                );
            }
            EmbedderMsg::NotifyLoadStatusChanged(_webview_id, status) => match status {
                ...
                _ => {
                    log::trace!(
                        "Eesha WebView {webview_id:?} ignores NotifyLoadStatusChanged status: {status:?}"
                    );
                }
            },
            ...
            EmbedderMsg::AllowNavigationRequest(_webview_id, id, url) => {
                if let Some(to_controller_sender) = to_controller_sender {
                    if self.event_listeners.on_navigation_starting {
                        ...
                        {
                            log::error!(
                                "Eesha failed to send AllowNavigationRequest to controller: {error}"
                            )
                        } else {
                            // We will handle a ToEeshaMessage::OnNavigationStartingResponse
                ...
                // If it's in Eesha Browser, check if we should download the url
                if self.panel.is_some() {
                    let sender = sender.clone();
                    let url = url.into_url();
                    let client = self.reqwest_client.clone();
                    let eesha_internal_sender = self.eesha_internal_sender.clone();

                    tokio::spawn(async move {
                        let (should_download, resp) = check_should_download(&client, &url).await;
                        if should_download && resp.is_some() {
                            download_body(url, resp.unwrap(), eesha_internal_sender).await;
                        } else {
                            send_to_constellation(
                                &sender,
                    ...
                    // If it's not in Eesha Browser, just allow the navigation
                    send_to_constellation(
                        sender,
                        EmbedderToConstellationMessage::AllowNavigationResponse(id, true),
                    );
                    ...
                    if let Some(request_map) = &mut self.event_listeners.on_web_resource_requested {
                        ...
                        match to_controller_sender.send(
                            ToControllerMessage::OnWebResourceRequested(
                                eesha_messages::WebResourceRequest {
                                    id,
                            ...
                            Ok(_) => {
                                ...
                                // We will handle a ToEeshaMessage::WebResourceRequestResponse
            ...
            EmbedderMsg::GetClipboardText(_webview_id, sender) => {
                let text = clipboard
                    .map(|c| {
                        c.get_text().unwrap_or_else(|e| {
                            log::warn!(
                                "Eesha WebView {webview_id:?} failed to get clipboard text: {}",
                                e
                            );
                            String::new()
                        })
                ...
                if let Err(e) = sender.send(Ok(text)) {
                    log::warn!(
                        "Eesha WebView {webview_id:?} failed to send clipboard text: {}",
                        e
                    );
                }
            }
            EmbedderMsg::SetClipboardText(_webview_id, text) => {
                if let Some(c) = clipboard {
                    if let Err(e) = c.set_text(text) {
                        log::warn!(
                            "Eesha WebView {webview_id:?} failed to set clipboard text: {}",
                            e
                        );
                    }
                }
                ...
                if let Some(tab) = self.tab_manager.tab(webview_id) {
                    ...
                    match simple_dialog {
                        ...
                        SimpleDialog::Prompt {
                            ...
                            if message.starts_with("EESHA::") {
                                self.handle_eesha_internal_messages_with_webview(
                                    message.strip_prefix("EESHA::").unwrap(),
                                    response_sender,
                                    sender,
                                    tab,
                                );
            ...
            e => {
                log::trace!("Eesha WebView isn't supporting this message yet: {e:?}")
            }
        }
    }
    ...
    pub fn handle_servo_messages_with_panel(
        ...
        log::trace!("Eesha Panel {panel_id:?} is handling Embedder message: {message:?}",);
        match message {
            EmbedderMsg::WebViewClosed(_) => {
                // Most WebView messages are ignored because it's done by compositor.
                log::trace!("Eesha Panel ignores this message: {message:?}")
            }
            EmbedderMsg::WebViewBlurred => {
                self.focused_webview_id = None;
            }
            EmbedderMsg::WebViewFocused(webview_id) => {
                ...
                self.close_webview_menu(&sender);
                log::debug!(
                    "Eesha Window {:?}'s panel {} has loaded completely.",
                    self.id(),
                    webview_id
                );
            }
            EmbedderMsg::NotifyLoadStatusChanged(_webview_id, status) => {
                if status == LoadStatus::Complete {
                    ...
                    log::trace!("Eesha Panel ignores NotifyLoadStatusChanged status: {status:?}");
                }
            }
                    ...
                    SimpleDialog::Prompt {
                        ...
                        if message.starts_with("CLOSE_TAB:") {
                            ...
                            if let Some(tab) = self.tab_manager.current_tab() {
                                if let Some(url) = tab.history().list.get(tab.history().current_idx)
                                {
                                    ...
                                    // Ignore the bookmark if it starts with "eesha://"
                                    if url.starts_with("eesha://") {
                                        return false;
                                    }
                        ...
                        match message.as_str() {
                            ...
                            "DOWNLOAD" => {
                                self.create_tab(
                                    &sender,
                                    ServoUrl::parse("eesha://resources/components/downloads.html")
                                        .unwrap(),
                                );
                            }
                            _ => {}
                        ...
                        if let Some(tab) = self.tab_manager.current_tab() {
                            ...
                            if message.starts_with("NAVIGATE_TO:") {
                                ...
                                let url = match Url::parse(unparsed_url) {
                                    ...
                                    Err(e) => {
                                        if e == url::ParseError::RelativeUrlWithoutBase {
                                            ...
                                            panic!("Eesha Panel failed to parse URL: {}", e);
                                        }
                                    }
                                };
                                ...
                                tokio::spawn(async move {
                                    ...
                                    if should_download && resp.is_some() {
                                        download_body(url, resp.unwrap(), eesha_internal_sender)
                                            .await;
                                    } else {
                                        send_to_constellation(
                                            &sender,
                                ...
                                self.create_tab(
                                    &sender,
                                    ServoUrl::parse("eesha://resources/components/bookmark.html")
                                        .unwrap(),
                                );
                            } else {
                                match message.as_str() {
                                    ...
                                    e => log::trace!(
                                        "Eesha Panel isn't supporting this prompt message yet: {e}"
                                    ),
                                }
                            }
                        }
                    }
                    _ => log::trace!("Eesha Panel isn't supporting this prompt yet"),
                }
            }
            EmbedderMsg::GetClipboardText(_webview_id, sender) => {
                let text = clipboard
                    .map(|c| {
                        c.get_text().unwrap_or_else(|e| {
                            log::warn!("Eesha Panel failed to get clipboard text: {}", e);
                            String::new()
                        })
                    })
                    .unwrap_or_default();
                if let Err(e) = sender.send(Ok(text)) {
                    log::warn!("Eesha Panel failed to send clipboard text: {}", e);
                }
            }
            EmbedderMsg::SetClipboardText(_webview_id, text) => {
                if let Some(c) = clipboard {
                    if let Err(e) = c.set_text(text) {
                        log::warn!("Eesha Panel failed to set clipboard text: {}", e);
                    }
                }
            }
            ...
            e => {
                log::trace!("Eesha Panel isn't supporting this message yet: {e:?}")
            }
        }
        false
    }
    ...
    pub fn handle_servo_messages_with_webview_menu(
        ...
        log::trace!("Eesha WebView Menu {webview_id:?} is handling Embedder message: {message:?}",);
        match message {
            EmbedderMsg::WebViewBlurred => {
                self.focused_webview_id = None;
            }
            ...
            EmbedderMsg::ShowSimpleDialog(_webview_id, simple_dialog) => match simple_dialog {
                ...
                _ => log::trace!("Eesha context menu isn't supporting this prompt yet"),
            },
            #[cfg(linux)]
            EmbedderMsg::ShowContextMenu(_webview_id, servo_sender, _title, _options) => {
                if self.webview_menu.is_none() {
            ...
            e => {
                log::trace!("Eesha context menu isn't supporting this message yet: {e:?}")
            }
        }
        false
    }
    ...
    pub fn handle_servo_messages_with_prompt(
        ...
        log::trace!("Eesha Prompt {webview_id:?} is handling Embedder message: {message:?}",);
        match message {
            EmbedderMsg::WebViewBlurred => {
                self.focused_webview_id = None;
            }
            ...
            e => {
                log::trace!("Eesha Dialog isn't supporting this message yet: {e:?}")
            }
        }
        false
    }
    /// Handle Eesha internal messages with webview.
    pub fn handle_eesha_internal_messages_with_webview(
        &self,
        message: &str,
        ...
        // If the prompt is come from Eesha's Downloads or Bookmark page, update the status
        if message == "DOWNLOAD_STATUS_GET" {
            let _ = self
                .eesha_internal_sender
                .send(EeshaInternalMsg::UpdateDownloadsPage(response_sender));
            ...
            if let Some(id) = message.strip_prefix("ABORT_DOWNLOAD::") {
                ...
                let _ = self
                    .eesha_internal_sender
                    .send(EeshaInternalMsg::AbortDownload(
            ...
            let _ = self
                .eesha_internal_sender
                .send(EeshaInternalMsg::UpdateBookmarkManager(response_sender));
            return;
            ...
            if let Some(id) = message.strip_prefix("BOOKMARK_RENAME::") {
                ...
                let _ = response_sender.send(PromptResponse::Cancel);
                let _ = self.eesha_internal_sender.send(
                    if let Ok(id) = BookmarkId::from_str(params[0]) {
                        EeshaInternalMsg::BookmarkRename(id, params[1].to_string())
                    } else {
