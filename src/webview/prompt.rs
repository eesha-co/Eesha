...
use crate::{app::send_to_constellation, webview::WebView};
    ...
    fn resource_url(&self, prompt_type: PromptType) -> ServoUrl {
        let url = match prompt_type {
            PromptType::Alert(msg) => {
                format!("eesha://resources/components/prompt/alert.html?msg={msg}")
            }
            PromptType::OkCancel(msg) => {
                format!("eesha://resources/components/prompt/ok_cancel.html?msg={msg}")
            }
            PromptType::AllowDeny(msg) => {
                format!("eesha://resources/components/prompt/allow_deny.html?msg={msg}")
            }
            PromptType::Input(msg, default_value) => {
                let mut url = format!("eesha://resources/components/prompt/prompt.html?msg={msg}");
            ...
            PromptType::HttpBasicAuth => {
                "eesha://resources/components/prompt/http_basic_auth.html".to_string()
            }
        };
