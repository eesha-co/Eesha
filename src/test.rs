/// Simple test harness for testing `eesha`.
///
...
/// ```
///
/// Then, in your test, use the `eesha_test!` macro to run your tests. The tests must be functions that take an `EventLoopWindowTarget`.
///
/// ```rust
/// use eesha::eesha_test;
/// use eesha::winit::event_loop::EventLoopWindowTarget;
///
...
/// }
///
/// eesha_test!(my_test, other_test);
/// ```
#[macro_export]
macro_rules! eesha_test {
    ($($test:expr),*) => {
        fn main() -> Result<(), Box<dyn std::error::Error>> {
            const TESTS: &[$crate::test::__private::EeshaBasedTest] = &[$(
                $crate::__eesha_test_internal_collect_test!($test)
            ),*];
...
#[doc(hidden)]
#[macro_export]
macro_rules! __eesha_test_internal_collect_test {
    ($name:expr) => {
        $crate::test::__private::EeshaBasedTest {
            name: stringify!($name),
...
pub mod __private {
    ...
    pub fn run(tests: &'static [EeshaBasedTest], _ctx: Context) {
        ...
        let mut eesha = Eesha::new(window, event_loop.create_proxy());
        ...
        // Run the tests.
        event_loop
            .run(move |event, elwt| match eesha.run(event, elwt) {
                Status::LoadComplete => {
