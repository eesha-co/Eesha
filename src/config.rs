...
use eesha_messages::{ConfigFromController, UserScript};
...
pub struct CliArgs {
    ...
    /// Servo time profile settings
    pub profiler_settings: Option<eesha_messages::ProfilerSettings>,
...
pub fn parse_cli_args() -> Result<CliArgs, getopts::Fail> {
    ...
    opts.optopt(
        "",
        "ipc-channel",
        "IPC channel name to communicate and control eesha",
        "",
    );
    opts.optflag("", "no-panel", "Launch Eesha without control panel");
    opts.optopt(
        "",
        "devtools-port",
        "Launch Eesha with devtools server enabled and listen to port",
        "1234",
    );
    opts.optopt(
        "p",
        "profiler",
        "Launch Eesha with servo time profiler enabled and output to stdout with an interval",
        "5",
    );
    opts.optopt(
        "",
    ...
    let profiler_settings = if let Ok(Some(profiler_interval)) = matches.opt_get("profiler") {
        ...
        Some(eesha_messages::ProfilerSettings {
            output_options: if let Some(output_file) = profile_output {
                eesha_messages::OutputOptions::FileName(output_file)
            } else {
                eesha_messages::OutputOptions::Stdout(profiler_interval)
            },
...
impl Config {
    /// Create a new configuration for creating Eesha instance from the CLI arguments.
    ...
    /// Create a new configuration for creating Eesha instance from the controller config.
    pub fn from_controller_config(config: ConfigFromController) -> Self {
            ...
            config
                ...
                .map(|profiler_settings| ProfilerSettings {
                    output_options: match profiler_settings.output_options {
                        eesha_messages::OutputOptions::FileName(outfile) => {
        ...
        Self {
            url: config
                .url
                .unwrap_or_else(|| url::Url::parse("eesha://newtab").unwrap()),
            with_panel,
            window_attributes,
    ...
    pub fn create_protocols(&self) -> ProtocolRegistry {
        ...
        protocols.register("eesha", handler);
        protocols
    }
...
impl ProtocolHandler for ResourceReader {
    fn load(
        ...
        // If the path has no file extension, try appending .html as a fallback.
        // This allows cleaner URLs like eesha://newtab to resolve to newtab.html.
