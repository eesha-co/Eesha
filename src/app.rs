...
use style;
use eesha_messages::{PositionType, SizeType, ToControllerMessage, ToEeshaMessage};
use webgpu;
...
/// Main entry point of Eesha browser.
pub struct Eesha {
...
/// Message for Eesha internal communication
#[derive(Serialize, Deserialize)]
pub enum EeshaInternalMsg {
    /// Abort a download
    AbortDownload(DownloadId),
    /// Create a download state in Eesha.
...
impl Debug for EeshaInternalMsg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EeshaInternalMsg::AbortDownload(_) => write!(f, "AbortDownload"),
            EeshaInternalMsg::CreateDownload(_) => write!(f, "CreateDownload"),
            EeshaInternalMsg::UpdateDownload(_, _) => write!(f, "UpdateDownload"),
            EeshaInternalMsg::UpdateDownloadsPage(_) => write!(f, "UpdateDownloadsPageStatus"),
            EeshaInternalMsg::UpdateBookmarkManager(_) => write!(f, "UpdateBookmarkManager"),
            EeshaInternalMsg::BookmarkRemove(_) => write!(f, "BookmarkRemove"),
            EeshaInternalMsg::BookmarkRename(_, _) => write!(f, "BookmarkRename"),
        }
    }
}

impl Eesha {
    /// Create a Eesha instance from Winit's window and event loop proxy.
    ///
    /// Following threads will be created while initializing Eesha based on configurations:
    ...
    pub fn new(evl: &ActiveEventLoop, proxy: EventLoopProxy<EventLoopProxyMessage>) -> Self {
        ...
        let (mut window, rendering_context) =
            Window::new(evl, window_settings, eesha_internal_sender.clone());
        ...
        ROUTER.add_typed_route(
            eesha_internal_receiver,
            Box::new(move |message| match message {
                Ok(message) => {
                    if let Err(e) =
                        proxy_clone.send_event(EventLoopProxyMessage::EeshaInternalMessage(message))
                    {
                        log::error!("Failed to send controller message to Eesha: {e}");
                    }
                }
    ...
    /// Initialize Eesha instance.
    ///
    /// This function is called when the Eesha instance is created.
    ...
    ///
    /// This function is called when the Eesha instance is shutting down.
    ...
    fn handle_winit_window_event(&mut self, window_id: WindowId, event: WindowEvent) -> bool {
        log::trace!("Eesha is handling Winit event: {event:?}");
        ...
        if let WindowEvent::CloseRequested = event {
            if let Some(to_controller_sender) = &self.to_controller_sender {
                if window.event_listeners.on_close_requested {
                    ...
                    {
                        log::error!(
                            "Eesha failed to send WebResourceRequested to controller: {error}"
                        );
                    } else {
                        return false;
                    }
    ...
    pub fn handle_servo_messages(&mut self, evl: &ActiveEventLoop) {
        if self.compositor.is_none() {
            log::error!("Eesha shouldn't be handling messages after compositor has shut down");
            return;
        }
            ...
            ShutdownState::NotShuttingDown => {
                for msg in messages {
                    if let Some(webview_id) = Self::get_embedder_message_webview_id(&msg) {
                        ...
                        // Handle message in Eesha Window
                        log::trace!("Eesha Window is handling Embedder message: {msg:?}");
                        match msg {
                            ...
                            e => {
                                log::trace!(
                                    "Eesha Window isn't supporting handling this message yet: {e:?}"
                                )
                            }
                        }
                    }
        ...
        // Check if Eesha need to start shutting down.
    ...
    /// Handle message from the Eesha internal channel
    pub fn handle_eesha_internal_message(&mut self, message: EeshaInternalMsg) {
        match message {
            EeshaInternalMsg::AbortDownload(id) => {
                let download = self.downloads.get_mut(&id);
                if let Some(download) = download {
                    download.abort();
                }
            }
            EeshaInternalMsg::CreateDownload(download) => {
            ...
            EeshaInternalMsg::UpdateDownload(id, new_state) => {
            ...
            EeshaInternalMsg::UpdateDownloadsPage(sender) => {
            ...
            EeshaInternalMsg::UpdateBookmarkManager(sender) => {
    ...
    /// Request Eesha to redraw. It will queue a redraw event on current focused window.
    ...
    pub fn handle_incoming_webview_message(&mut self, message: ToEeshaMessage) {
        match message {
            ToEeshaMessage::Exit => {
                if let Some(compositor) = &mut self.compositor {
                    compositor.maybe_start_shutting_down();
            ...
            ToEeshaMessage::ListenToOnNavigationStarting => {
                if let Some(window) = self.first_window_mut() {
                    window.event_listeners.on_navigation_starting = true;
                }
            }
            ToEeshaMessage::OnNavigationStartingResponse(id, allow) => {
                send_to_constellation(
                    &self.constellation_sender,
            ...
            ToEeshaMessage::ExecuteScript(js) => {
                if let Some(webview_id) = self.first_webview_id() {
                    let _ = execute_script(&self.constellation_sender, &webview_id, js);
                }
            }
            ToEeshaMessage::ListenToWebResourceRequests => {
                if let Some(window) = self.first_window_mut() {
                    window
                        .event_listeners
            ...
            ToEeshaMessage::WebResourceRequestResponse(response) => {
                if let Some(window) = self.first_window_mut() {
            ...
            ToEeshaMessage::SetVisible(visible) => {
                if let Some(window) = self.first_window() {
                    window.window.set_visible(visible);
                }
            }
            ToEeshaMessage::StartDragging => {
                if let Some(window) = self.first_window() {
                    let _ = window.window.drag_window();
                }
            }
            ToEeshaMessage::Focus => {
                if let Some(window) = self.first_window() {
                    window.window.focus_window();
                }
            }
            ToEeshaMessage::GetSize(id, size_type) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!("Eesha failed to send GetSizeReponse to controller: {error}")
                    }
                }
            }
            ToEeshaMessage::GetPosition(id, position_type) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!(
                            "Eesha failed to send GetPositionResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetMinimized(id) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!(
                            "Eesha failed to send GetMinimizedResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetMaximized(id) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ToControllerMessage::GetMaximizedResponse(id, window.window.is_maximized()),
                    ) {
                        log::error!(
                            "Eesha failed to send GetMaximizedResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetFullscreen(id) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!(
                            "Eesha failed to send GetFullscreenResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetVisible(id) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!(
                            "Eesha failed to send GetVisibleResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetScaleFactor(id) => {
                if let Some(window) = self.first_window() {
                    if let Err(error) = self.to_controller_sender.as_ref().unwrap().send(
                        ...
                        log::error!(
                            "Eesha failed to send GetScaleFactorResponse to controller: {error}"
                        )
                    }
                }
            }
            ToEeshaMessage::GetCurrentUrl(id) => {
                if let Some(window) = self.first_window() {
    ...
    /// Return true if one of the Eesha windows is animating.
...
/// then return the config from [`ToEeshaMessage::SetConfig`] or fallback to from the command line arguments
fn try_connect_ipc_and_get_config(
    ...
    let (to_controller_sender, initial_settings) = if let Some(ipc_channel) = &cli_args.ipc_channel
    {
        ...
        let (to_eesha_sender, receiver) = ipc::channel::<ToEeshaMessage>().unwrap();
        sender
            .send(ToControllerMessage::SetToEeshaSender(to_eesha_sender))
            .unwrap();
        let ToEeshaMessage::SetConfig(initial_settings) = receiver
            .recv()
            .expect("Failed to recieve the initial settings from controller")
        else {
            panic!("The initial message sent from eesha is not a `ToEeshaMessage::SetConfig`")
        };
        let proxy_clone = proxy.clone();
        ROUTER.add_typed_route(
            receiver,
...
pub enum EventLoopProxyMessage {
    ...
    IpcMessage(Box<ToEeshaMessage>),
    /// Message coming from the internal channel
    EeshaInternalMessage(EeshaInternalMsg),
}
...
impl EventLoopWaker for Waker {
    ...
    fn wake(&self) {
        if let Err(e) = self.0.send_event(EventLoopProxyMessage::Wake) {
            log::error!("Servo failed to send wake up event to Eesha: {e}");
        }
    }
}
