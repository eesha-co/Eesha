...
/// The Eesha compositor contains a GL rendering context with a WebRender instance.
...
impl IOCompositor {
    ...
    fn create_or_update_webview(
        ...
        debug!(
            "Eesha Compositor is setting frame tree with pipeline {} for webview {}",
    ...
    fn remove_webview(
        ...
        debug!("Eesha Compositor is removing webview {}", webview_id);
    ...
    pub fn swap_current_window(&mut self, window: &mut Window) {
        if window.id() != self.current_window {
            debug!(
                "Eesha Compositor swap current window from {:?} to {:?}",
                self.current_window,
                window.id()
            );
