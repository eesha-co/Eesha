...
use eesha_messages::{ConfigFromController, ProfilerSettings, UserScript};

use crate::EeshaViewController;

/// A builder for configuring and creating a [`EeshaViewController`] instance.
#[derive(Debug, Clone)]
pub struct EeshaBuilder(ConfigFromController);

impl EeshaBuilder {
    /// Creates a new [`EeshaBuilder`] with default settings.
    pub fn new() -> Self {
    ...
    pub fn icon(mut self, icon: eesha_messages::Icon) -> Self {
    ...
    /// Builds the [`EeshaViewController`] with the configured settings.
    pub fn build(
        self,
        eesha_path: impl AsRef<Path>,
        initial_url: url::Url,
    ) -> EeshaViewController {
        EeshaViewController::create(eesha_path, initial_url, self.0)
    }
}
