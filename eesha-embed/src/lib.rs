...
pub use eesha_messages::{
    ConfigFromController as EeshaSettings, Icon, ProfilerSettings, UserScript,
};
use eesha_messages::{
    PositionType, SizeType, ToControllerMessage, ToEeshaMessage, WebResourceRequestResponse,
};
...
/// A EeshaView controller
///
/// Send an exit signal to this Eesha instance when dropped
pub struct EeshaViewController {
    sender: IpcSender<ToEeshaMessage>,
    event_listeners: EventListeners,
}

impl EeshaViewController {
    /// Create a new eesha instance with settings and get the controller to it
    fn create(
        eesha_path: impl AsRef<Path>,
        initial_url: url::Url,
        mut settings: EeshaSettings,
    ) -> Self {
        let path = eesha_path.as_ref();
        ...
        let ToControllerMessage::SetToEeshaSender(sender) = message else {
            panic!(
                "The initial message sent from eesha is not a `ToControllerMessage::SetToEeshaSender`"
            )
        };

        settings.url.replace(initial_url);
        sender
            .send(ToEeshaMessage::SetConfig(settings))
            .expect("Failed to send initial settings to eesha");
        ...
        let to_eesha_sender = sender.clone();
        ROUTER.add_typed_route(
            ...
            Box::new(move |message| match message {
                Ok(message) => match message {
                    ...
                    ToControllerMessage::OnNavigationStarting(id, url) => {
                        if let Some(ref callback) = *on_navigation_starting.lock().unwrap() {
                            if let Err(error) = to_eesha_sender.send(
                                ToEeshaMessage::OnNavigationStartingResponse(id, callback(url)),
                            ) {
                                error!(
                    ...
                    ToControllerMessage::OnWebResourceRequested(request) => {
                        if let Some(ref callback) = *on_web_resource_requested.lock().unwrap() {
                            let sender_clone = to_eesha_sender.clone();
                            let id = request.id;
                            callback(
                                request.request,
                                Box::new(move |response| {
                                    if let Err(error) = sender_clone.send(ToEeshaMessage::WebResourceRequestResponse(
                ...
                Err(e) => error!("Error while receiving EeshaMessage: {e}"),
            }),
        );
        Self {
            sender,
    ...
    /// Create a new eesha instance with default settings and get the controller to it
    pub fn new(eesha_path: impl AsRef<Path>, initial_url: url::Url) -> Self {
        Self::create(eesha_path, initial_url, EeshaSettings::default())
    }

    /// Exit
    pub fn exit(&self) -> Result<(), Box<ipc_channel::ErrorKind>> {
        self.sender.send(ToEeshaMessage::Exit)
    }

    /// Listen on close requested from the OS,
    /// if you decide to use it, eesha will not close the window by itself anymore,
    ...
    pub fn on_close_requested(
        ...
        if old_listener.is_none() {
            self.sender.send(ToEeshaMessage::ListenToOnCloseRequested)?;
        }
        Ok(())
    }
    ...
    pub fn execute_script(&self, script: String) -> Result<(), Box<ipc_channel::ErrorKind>> {
        self.sender.send(ToEeshaMessage::ExecuteScript(script))
    }

    /// Navigate to url
    pub fn navigate(&self, url: url::Url) -> Result<(), Box<ipc_channel::ErrorKind>> {
        self.sender.send(ToEeshaMessage::NavigateTo(url))
    }

    /// Reload the current webview
    pub fn reload(&self) -> Result<(), Box<ipc_channel::ErrorKind>> {
        self.sender.send(ToEeshaMessage::Reload)
    }
    ...
    pub fn on_navigation_starting(
        ...
        if old_listener.is_none() {
            self.sender
                .send(ToEeshaMessage::ListenToOnNavigationStarting)?;
        }
        Ok(())
    }
    ...
    pub fn on_web_resource_requested(
        ...
        if old_listener.is_none() {
            self.sender
                .send(ToEeshaMessage::ListenToWebResourceRequests)?;
        }
        Ok(())
    }
    ...
    pub fn set_size<S: Into<Size>>(&self, size: S) -> Result<(), Box<ipc_channel::ErrorKind>> {
        self.sender.send(ToEeshaMessage::SetSize(size.into()))?;
        Ok(())
    }
    ...
    fn get_size(
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetSize(id, size_type)) {
            self.event_listeners
                .size_response
                .lock()
                .unwrap()
    ...
    fn get_position(
        ...
        if let Err(error) = self
            .sender
            .send(ToEeshaMessage::GetPosition(id, position_type))
        {
            self.event_listeners
                .position_response
                .lock()
    ...
    pub fn is_maximized(&self) -> Result<bool, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetMaximized(id)) {
            self.event_listeners
                .maximized_response
                .lock()
                .unwrap()
    ...
    pub fn is_minimized(&self) -> Result<bool, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetMinimized(id)) {
            self.event_listeners
                .minimized_response
                .lock()
                .unwrap()
    ...
    pub fn is_fullscreen(&self) -> Result<bool, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetFullscreen(id)) {
            self.event_listeners
                .fullscreen_response
                .lock()
                .unwrap()
    ...
    pub fn is_visible(&self) -> Result<bool, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetVisible(id)) {
            self.event_listeners
                .visible_response
                .lock()
                .unwrap()
    ...
    pub fn get_scale_factor(&self) -> Result<f64, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetScaleFactor(id)) {
            self.event_listeners
                .scale_factor_response
                .lock()
                .unwrap()
    ...
    pub fn get_current_url(&self) -> Result<url::Url, Box<ipc_channel::ErrorKind>> {
        ...
        self.event_listeners
            ...
            .unwrap()
            .insert(id, sender);
        if let Err(error) = self.sender.send(ToEeshaMessage::GetCurrentUrl(id)) {
            self.event_listeners
                .get_url_response
                .lock()
                .unwrap()
    ...
    //     self.sender.send(ToEeshaMessage::AddInitScript(script))
    // }
}

impl Drop for EeshaViewController {
    /// Send an exit signal to this Eesha instance
    fn drop(&mut self) {
        let _ = self.exit();
    }
}
